...
import { AxisMergeButton } from '../axis-merge-button'
...
export default function Graph({
	...
	function handleMergeAxes() {
		const mergedParts = sensors.map((s) => `${s.deviceId}/${s.id}/1`).join('/')
		void navigate({
				...
				tooltip: {
					...
					callbacks: {
						title: (tooltipItems: any[]) => {
							...
							if (!firstItem) return ''
							return dateTimeFormatter.format(new Date(firstItem.raw.x))
						},
						label: (context: any) => {
				...
				>
					...
					>
						<div className="flex grow flex-wrap items-center gap-2">
							...
							<AxisMergeButton
								canMerge={canMerge}
								handleMergeAxes={handleMergeAxes}
								axisUnits={[axisUnits['1']]}
							/>
						</div>
